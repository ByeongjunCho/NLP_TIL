...
class PositionWiseFeedForward(nn.Module):
    ...
    def __init__(self, cfg):
        super().__init__()
        self.fc1 = nn.Linear(cfg.dim, cfg.dim_ff)  # FeedForward1
        self.fc2 = nn.Linear(cfg.dim_ff, cfg.dim)  # FeedForward2
...
class Block(nn.Module):
    ...
    def __init__(self, cfg):
        super().__init__()
        self.attn = MultiHeaderSelfAttention(cfg)
    ...
    def forward(self, x, mask):
        h = self.attn(x, mask)
        h = self.norm1(x + self.drop(self.proj(h))) # Multi-head Attention + Residual
        h = self.norm2(h + self.drop(self.pwff(h))) # FeedForward + Residual
        return h
...
class Transformer(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.block = nn.ModuleList([Block(cfg) for _ in range(cfg.n_layers)])
